...
# warn also RECORDS (WARN_LOG, set by provision.sh like SOFT_FAIL_LOG) so the
# end-of-run summary can list what was tolerated — a warning scrolled past
# 20 minutes ago is otherwise as good as swallowed (2026-09-22, assessment).
WARN_LOG="${WARN_LOG:-}"
warn() {
...
# A bring-to-latest updater that failed (rustup update, brew update): on a
# normal run the box just stays where it was — warn and carry on. Under
# GOLDEN_IMAGE that staleness would be baked into every clone, so it is a
# real failure there (soft_fail → die). Added 2026-09-22 (assessment): those
# sites were `|| true`, so a stale golden could never be noticed.
stale_warn() {
...
# apt_preflight STOPS the apt-daily timers for the run (see above) and, until
# 2026-09-22, left them stopped until the next reboot — a normal provision
# ended with unattended-upgrades switched off for days (assessment). Restart
# them at the end of a non-golden run; a golden build is captured/powered off
# and the enabled timers return at the clone's first boot on their own.
apt_preflight_restore() {
  if dry; then would "restart apt-daily.timer + apt-daily-upgrade.timer (paused by the preflight for this run)"; return 0; fi
  command -v systemctl >/dev/null 2>&1 || return 0
  $SUDO systemctl start apt-daily.timer apt-daily-upgrade.timer 2>/dev/null \
    || warn "could not restart the apt-daily timers — they resume at next boot"
}
