...
# .zshrc), needs no Node, and self-updates in place. `latest` is the channel.
set -uo pipefail
source "$(dirname "${BASH_SOURCE[0]}")/../lib.sh"

log "Claude Code (native installer, latest channel) for '$TARGET_USER'"
...
# Channel: `latest`, not `stable` (switched 2026-09-13). Claude's own updater
# moves an installed copy to latest regardless, so a `stable` golden (2.1.236
# that day) merely started every clone 34 versions behind where its first
# `claude update` put it (2.1.270). versions.lock records what landed.
as_user "test -x \"\$HOME/.local/bin/claude\" || {
  t=\$(mktemp) && bash '$PINS' fetch_pinned '$CLAUDE_BOOTSTRAP_URL' '$CLAUDE_BOOTSTRAP_SHA256' \"\$t\" \
    && bash \"\$t\" latest; rc=\$?; rm -f \"\$t\"; exit \$rc; }" \
