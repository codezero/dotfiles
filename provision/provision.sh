...
if ! dry; then
  ...
  WARN_LOG="$(mktemp)" || die "could not create warning log (mktemp failed)"
  export WARN_LOG
...
# Warnings are tolerated by design (stale updaters, a flaky vendor index, kept
# files) — but list them, so "no failures" never hides "and three things were
# skipped". Dry runs warn about nothing worth repeating.
if ! dry && [ -n "${WARN_LOG:-}" ] && [ -s "$WARN_LOG" ]; then
  warn_n="$(wc -l < "$WARN_LOG" | tr -d ' ')"
  printf '\033[1;33m[warn]\033[0m %s\n' "$warn_n warning(s) during this run (tolerated, not failures):" >&2
  sort -u "$WARN_LOG" | sed 's/^/    - /' >&2
fi
