...
- Step 10 filters boot/kernel/third-party/desktop out of `apt.list` at install time (the list is the full `apt-mark showmanual` set on purpose); step 20 owns Docker/VSCodium/Bruno/Cursor in **key → verify → repo → install** order — those packages have no repo at step 10. The repo add + install are **gated on a valid signing key** (`verify_keyring`): Docker, Cursor, and VSCodium fingerprints are pinned (`DOCKER_FP`/`CURSOR_FP`/`VSCODIUM_KEY_FP`, each verified against the live key); only Bruno is verified-but-unpinned — its key rotates and the keyserver returns multiple keys (`usebruno#3569`), so a pin would break (set `BRUNO_KEY_FP` to pin it yourself). A failed/mismatched key `soft_fail`s and skips that vendor rather than adding a half-configured repo.
...
- Arch-aware fallbacks: Docker codename falls back to `noble`; Bruno's apt repo is amd64-only so arm64 pulls a GitHub `arm64` .deb. Cursor uses its signed apt repo (`downloads.cursor.com/aptrepo`, suite `stable`, both arches — codename-agnostic).
- Bruno's arm64 `.deb` download uses `mktemp` (0600), not a predictable `/tmp` path, to avoid a TOCTOU swap of the file apt installs as root.
