...
- [ ] **Golden-image finalize hardening (commit `46a2b87`) is dry-run-verified only** — exercise on a throwaway `sudo GOLDEN_IMAGE=1` build, ideally with creds/keys *planted* first to prove the wipe + gate actually fire: the **STRICT verification gate** in step 90 (must `die` if `/etc/machine-id` non-empty, SSH host keys, cred dirs, or any `~/.ssh` private key remain — i.e. a deliberately-dirty image is rejected); the **broadened `~/.ssh` private-key sweep** (`find … -exec grep 'PRIVATE KEY' … -delete`, catching custom-named keys, not just `id_*`); the **`.docker/config.json` auth scrub** (jq `del(.auths,.credsStore,.credHelpers,.HttpHeaders)`, keep `currentContext`; drop the file if unscrubbable); and the **GOLDEN→forced-STRICT abort + post-finalize early-exit** in `provision.sh` (nothing prints/writes after step 90).
