...
## Open TODOs
- [x] **Live arm64 smoke-test — DONE 2026-05-25** (fresh Ubuntu 26.04 arm64). **Phase A** (normal `sudo bash provision/provision.sh`): clean ✅ first run + clean idempotent re-run — exercised apt base + Docker/VSCodium/Cursor signed repos, `brew bundle`, `rustup`, `cargo install alacritty` (0.17.0), the Claude installer, vendored MesloLGS NF + `fc-cache`, the shell-completions fetch, dotfile symlinks, `chsh`. **Phase B** (`sudo GOLDEN_IMAGE=1 …`): STRICT run + copy-mode dotfiles + finalize, with **gate PASS** (clean wipe → exit 0) AND **gate ABORT** proven (an un-removable `chattr +i` cred → `die … image not clean`, exit 1); also confirmed the broadened `~/.ssh` sweep removes a *custom-named* private key, the `.docker/config.json` scrub keeps `currentContext`, machine-id/host-keys wiped, and `authorized_keys` kept when cloud-init is disabled. Three bugs the live run caught that dry-run + `shellcheck` both missed: `as_user` login-shell `~/.bash_logout`→`clear_console` exit-status corruption (`0fa9b9c`), `--dry-run` printing the follow-ups block (`0fa9b9c`), and completions installed 0600 → zsh `compinit` "permission denied" (`92d247b`).
- [ ] **Two opt-in paths still un-exercised live:** `INSTALL_DESKTOP=1` (desktop/locale/IME apt set + GNOME `dconf load`, step 55) and `DOCKER_ROOTLESS=1` (step 25 — headless `dockerd-rootless-setuptool.sh` + linger; needs unprivileged userns). The Phase A/B runs used neither.
